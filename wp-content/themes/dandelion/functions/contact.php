...
function pexeto_send_email(){
	...
	if($validated) {
		...
		if(isset($_POST["name"]) && $_POST["name"] && isset($_POST["email"]) && $_POST["email"] && isset($_POST["question"]) && $_POST["question"]){
			...
			$message = "contact form message from Bloompost.com \n
$name's email address is: $eeemail \r\n\r\n
do not reply directly to this email. Create a new email addressed to $eeemail \r\n\r\n
Question: $notes \r\n";
			
			$from = "From: ".$_POST["email"];
			
			$emailToSend=get_opt('_email');
			$headers = 'From: '.$name.' <no-reply@bloompost.com>' . "\r\n";
