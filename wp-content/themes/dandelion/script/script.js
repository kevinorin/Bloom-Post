...
pexetoSite = {
	...
	setDropDown:function(){
		...
		jQuery("#menu ul ul").data('padding', 9);
...
jQuery(document).ready(function($){
	...
	function backgroundFadeIn () { 
		$('#header-image-text').animate(					  
		{
			opacity:1
		},1500);
	}//fades in the top text
	function backgroundFadeIn2 () { 
		$('#header-button img').animate(
			{
				opacity:1
			},1500);
	}//fades in the button
	function MainBodyFadeIn () { 
		$('#mainContent, #outerHeader').animate(
			{
				opacity:1
			},1500);
	...
	jQuery(window)
		.load(function() {
			$('#header-image-link img').queue(function () {
				if (isHome) //check if we're at the homepage

				{
					setTimeout(backgroundFadeIn, 1500);  //fade in the buildings first
					setTimeout(backgroundFadeIn2, 4000);  //fade in the red building second
					//setTimeout(MainBodyFadeIn, 7000);  //fade in the text third
												   
					$(this).dequeue();
				} else {
					//$('#backgroundImage, #backgroundImage2, #mainContent, #outerHeader').css('opacity','1');
				}
			});
		});
