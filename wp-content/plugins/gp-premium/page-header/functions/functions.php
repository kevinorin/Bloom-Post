...
function generate_page_header_get_options( $id = false ) {
	...
	if ( is_tax() || is_category() || is_tag() && ! $id && ! is_singular() ) {
		$id = get_term_meta( get_queried_object()->term_id, '_generate-select-page-header', true );
	}
...
function generate_page_header_get_image_output() {
	...
	$width_upscale = $image_atts[ 'width' ] > $image_info[1] && $image_atts[ 'width' ] < 9999 ? true : false;

	// Is our height larger than the OG image and not proportional?
	$height_upscale = $image_atts[ 'height' ] > $image_info[2] && $image_atts[ 'height' ] < 9999 ? true : false;
	...
	if ( ! empty( $image_atts ) && 'enable' == $options[ 'image_resize' ] && function_exists( 'GP_Resize' ) ) {
		return '<img src="' . GP_Resize( $image_url, $image_atts[ 'width' ], $image_atts[ 'height' ], $crop, true, $upscale ) . '" alt="' . the_title_attribute( 'echo=0' ) . '" itemprop="image" />';
	} else {
		return apply_filters( 'post_thumbnail_html', 
			wp_get_attachment_image( $image_id, apply_filters( 'generate_page_header_default_size', 'full' ), '', array( 'itemprop' => 'image' ) ),
			get_the_ID(),
...
function generate_page_header_enqueue() {
	...
	if ( '' !== $options[ 'background_video' ] && '' !== $options[ 'content' ] ) {
...
function generate_page_header_css() {
	...
	if ( '' !== $options[ 'custom_menu_colors' ] ) {
		...
		$css->set_selector( '.generate-merged-header #site-navigation:not(.toggled) .main-nav > ul > li > a:hover, .generate-merged-header #site-navigation:not(.toggled) .main-nav > ul > li > a:focus, .generate-merged-header #site-navigation:not(.toggled) .main-nav > ul > li.sfHover > a' );
...
function generate_page_header_template_tags( $content ) {
	...
	if ( is_singular() ) {
		...
		if ( strpos( $content, '{{custom_field' ) !== false ) {
			...
			if ( '_thumbnail_id' == $match ) {
				$id = get_post_meta( get_the_ID(), $match, true );
				if ( null !== $id ) {
					$search[] = '{{custom_field._thumbnail_id}}';
					$replace[] = wp_get_attachment_image( $id, apply_filters( 'generate_page_header_thumbnail_id_size', 'medium' ) );
				}
			}
		}
	}
