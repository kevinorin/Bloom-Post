tinymce.PluginManager.add( 'loremipsum', function( editor, url ) {
	...
	// Get values from available languages and push them to a select box
	function getValues() {
		...
		for (var i=0; i < arrSentences.length; i++) {
				...
				if(arrLoremIpsumLangs[i] != undefined) {
					
					push_array = {text: arrLoremIpsumLangs[i] + ': ' + tmpText, value: [i]};
					sentences.push(push_array);
				}
		}
	...
	// Generate sentences based on user options
	...
	function insert_link() {
		...
		editor.windowManager.open({
			...
			onsubmit: function (t) {
                ...
                var sentences = t.data.source_text;
                var amount = t.data.generated_amount;
                var formating = t.data.formatting;
                ...
                switch (formating) {
                        ...
                        case 'sentences': //unformatted

                                insertedText = loremIpsumGimmeSentences(amount, sentences);
                        break;
						
                        case 'paragraphs': //paragraphs
                                for (var counter = 0; counter < amount; counter++)        {
                                        countElem = Math.round((Math.random() * 5) + 3); //so many sentences in paragraph
                                        insertedText += '<p>' + loremIpsumGimmeSentences(countElem, sentences) + '</p>';
                                }
                        break;
                        ...
                        case 'lists': //list
                                ...
                                for (var counter = 0; counter < amount; counter++)  {
                                        ...
                                        insertedText += '<li>' + loremIpsumGimmeSentences(countElem, sentences) + '</li>';
                                }
                                insertedText += '</ul>';
                        break;
						
                        case 'ordered_lists': //orderedlist
                                insertedText += '<ol>';
                                for (var counter = 0; counter < amount; counter++)      {
                                        countElem = Math.round(Math.random() + 1); //so many sentences in listelement
                                        insertedText += '<li>' + loremIpsumGimmeSentences(countElem, sentences) + '</li>';
				...
				editor.execCommand('mceInsertContent', !1, insertedText);
			}
		});	
	}
